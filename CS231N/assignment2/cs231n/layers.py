...
def softmax_loss(x, y):
    ...
    #sumi=np.sum(scores,axis=1).reshape(-1,1)
    #scores/=sumi
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    x, gamma, beta, x_hat, mean, var, eps = cache  #这里直接用了矩阵的链式法则，事实上不是计算图了
    m = x.shape[0]
    dx_hat = dout*gamma
    d_var = np.sum(dx_hat*(x-mean),axis=0)*(-0.5)*np.power(var+eps,-3/2)
    d_mean = np.sum(dx_hat*(-1.0/np.sqrt(var+eps)),axis=0)+d_var*np.sum((-2*(x-mean)/m),axis=0)
    dx = dx_hat/np.sqrt(var+eps)+d_var*2*(x-mean)/m+d_mean/m
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    cache = (x, gamma, mean, var, eps, x_hat)
    pass
...
def conv_backward_naive(dout, cache):
    ...
    for n in range(N):
      for f in range(F):
        db[f] += np.sum(dout[n,f,:,:]) #第n个图，第f个卷积核计算出out后，sum得到db[f]
        for i in range(H_out):
          for j in range(W_out):
    ...
    dx = dx_pad[:,:,pad:pad+H,pad:pad+W] #去pad
...
def max_pool_forward_naive(x, pool_param):
    ...
    assert (H - pool_height) % stride == 0, "Stride and pool_height are not compatible"
    assert (W - pool_width) % stride == 0, "Stride and pool_width are not compatible"
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
      for c in range(C):
        for i in range(H_out):
          for j in range(W_out):
            index_max = np.argmax(x[n,c,i*stride:i*stride+pool_height,j*stride:j*stride+pool_width])
            index_h,index_w = np.unravel_index(index_max,(pool_height,pool_width))
            dx[n,c,i*stride+index_h,j*stride+index_w] += dout[n,c,i,j]
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    N, C, H, W =x.shape
    x=x.transpose(0,2,3,1).reshape(-1,C)  #N,H,W,C  NHW,C
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    N, C, H, W=x.shape
    x_group = x.reshape(N,G,C//G,H,W)  #每张图片的C个channel共分成G组
    ...
    out = gamma*x_hat +beta
    cache = (x,gamma,beta,G,x_hat,mean,var,eps)
